...
# Numpy is often used to load, maniputale and perprocess data.
import numpy as np
...
# An estimator is the front end to invoke traioning (fitting) and evaluation (inference).
...
# Here we use two data sets: one for training and one for ealuation.
# We have to tell the function how many batches of data (num_epochs) we want and how big each abtch should be.
x_train = np.array([1., 2., 3., 4.])
y_train = np.array([0., -1., -2., -3.])
x_eval = np.array([2., 5., 8., 1.])
y_eval = np.array([-1.01, -5.1, -7, 0.])
